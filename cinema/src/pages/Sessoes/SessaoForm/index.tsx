...
export const SessaoForm = () => {
    ...
    const loadData = async () => {
        try {
            ...
            if (id) {
                try {
                    const sessao = await sessaoService.getById(id as any);
                    setFormData({
                        horarioExibicao: sessao.horarioExibicao || '',
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (id) {
				await sessaoService.update(id as any, validatedData);
			} else {
			...
			if (error.issues) {
				...
				const errorMessage = error.response.data?.message || error.response.statusText || (id ? 'Erro ao atualizar sessão' : 'Erro ao agendar sessão');
				...
				alert(`Erro: ${error.message || (id ? 'Erro desconhecido ao atualizar sessão' : 'Erro desconhecido ao agendar sessão')}`);
