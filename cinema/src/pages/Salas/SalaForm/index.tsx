...
export const SalaForm = () => {
    ...
    useEffect(() => {
        const loadSala = async () => {
            if (!id) return;
            try {
                const sala = await salaService.getById(id as any);
                setFormData({
                    numero: Number(sala.numero || 0),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (id) {
                await salaService.update(id as any, validatedData);
            } else {
            ...
            if (error.issues) {
                ...
                const errorMessage = error.response.data?.message || error.response.statusText || (id ? 'Erro ao atualizar sala' : 'Erro ao cadastrar sala');
                ...
                alert(`Erro: ${error.message || (id ? 'Erro desconhecido ao atualizar sala' : 'Erro desconhecido ao cadastrar sala')}`);
